-- Aerospace module implements a client for the Aerospace socket.
-- @module Aerospace
...
local AEROSPACE_APP_ID = "bobko.aerospace"
...
local DEFAULT_CONFIG = {
	SOCKET_PATH = string.format("/tmp/%s-%s.sock", AEROSPACE_APP_ID, os.getenv("USER")),
...
function Aerospace:read_with_timeout(fd, max_bytes, timeout_ms)
	local result, err = poll.rpoll(fd, timeout_ms)
	...
	elseif result == nil then
		...
		return unistd.read(fd, max_bytes)
	end
end
...
function Aerospace:receive(maxBytes)
	...
	print("before rcv read")

	--local response, err = unistd.read(self.fd, maxBytes)
	local response, err = self:read_with_timeout(self.fd, maxBytes, 200)
...
function Aerospace:focused_window(callback)
	local args = {
		...
		"--focused",
		"--format",
		"%{window-title}%{window-id}%{app-bundle-id}%{app-pid}%{app-name}",
		"--json",
	}
	...
	local response = decode_response(self:receive(DEFAULT_CONFIG.EXTENDED_BUFFER_SIZE))
	local window = decode_response(response.stdout)
	if callback then
...
function Aerospace:list_workspace_names(switches, callback)
	...
	local query = { command = "", args = args, stdin = "" }
	self:send(query)
	local response = split(decode_response(self:receive(DEFAULT_CONFIG.MAX_BUFFER_SIZE)).stdout)
	if callback then
